...
最终发布通过 `npm run verify:release` 对照线上所有静态模块、OpenAPI、源码归档和健康版本，并核对 Cubelink；源码推送后在独立 Git 镜像 fetch、严格 fsck 并核对提交。输出保留在发布工作区。完整 GitLab/Gogs 总目标持续有效，私有包原生 Worker 构建、其他包格式与其余平台差距继续开发。
