...
| 仓库流水线配置、DAG/并行任务/调度、缓存、变量/密钥、npm/TS 构建             | v0.14 实现固定版本 JSON 配置、DAG、并行任务与依赖产物；v0.19 增加持久定时调度与权限取消；v0.20 增加项目变量/密钥及日志脱敏；v0.21 增加 R2 共享构建缓存；空间变量继承和云端 npm/TS 仍待开发                                                          |
