...
`GET /api/repos/:namespace/:repo/tree-updates?ref=<fixed-commit>&path=<directory>` returns `ref`, `path`, `updates` (name/date/commit), `complete`, and `limited`. Repeated requests advance at most 8 commits each, with a 5,000-commit traversal ceiling. Each namespace has up to eight 96 KiB progress slots keyed by snapshot and directory expiring after seven days; changed refs or paths restart the walk. Current repository read authorization applies. Budget limits or failures show a dash rather than invented dates. Navigation cancels subsequent requests. This cache is separate from the v0.39 root cache and is removed on repository deletion.
