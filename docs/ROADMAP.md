...
2. **完整 CI/CD**：定时流水线见 v0.19，项目变量与密钥见 v0.20；共享缓存见 v0.21；云端 TypeScript/npm 受限编译见 v0.22。继续补齐空间变量继承、包下载缓存、更广泛前端工具链。任务必须绑定确定的代码和配置版本，权限撤销后停止读取凭据或发布，提供失败恢复和真实部署证据。
