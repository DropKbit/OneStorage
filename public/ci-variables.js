...
export function variablePanel(root, variables, branch, h, options = {}) {
  ...
  const inheritedHTML = inherited.length
    ? i18nHTML`<section class="panel" aria-label="继承的空间变量"><div class="panelhead"><strong>继承的空间变量</strong></div><div class="detail-body"><p class="muted">项目定义优先于空间定义，同一层级精确环境优先于 *。暂停或失去授权的高优先级定义会阻止回退。这里只显示元数据，空间所有者可在空间管理中编辑。</p>${inherited.map((v) => i18nHTML`<div class="token-row" data-inherited-variable="${esc(v.key)}"><div><strong>${esc(v.key)}</strong> <span class="pill">${v.enabled && v.available ? i18nText("可继承") : i18nText("不可用")}</span><p>${esc(v.workspace)} · ${esc(v.environment)} · ${v.secret ? i18nText("密钥") : i18nText("普通变量")} · ${v.protected ? i18nText("仅受保护分支") : i18nText("允许未保护分支")}</p><p class="muted">分支：${v.refs.map(esc).join("、")} · 版本 ${v.revision}</p></div></div>`).join("")}</div></section>`
    : "";
  return {
    html: i18nHTML`${inheritedHTML}${options.workspace ? i18nText('<p class="hint">空间共享变量由所有者管理，供空间内项目显式选择使用。轮换或撤权会取消各项目已使用它的未完成工作流。</p>') : ""}<section class="panel" aria-label="CI 变量与密钥"><div class="panelhead"><strong>CI 变量与密钥</strong></div><div class="detail-body"><p class="muted">值加密保存，提交后不再显示。任务通过 variables 显式选择名称，environment 选择环境；项目定义优先于空间定义，同一层级精确环境优先于 *。密钥仅提供给当前分支提交的手动、推送或定时任务，MR 及其重试不可读取密钥。</p><pre>{ "variables": ["API_TOKEN"], "environment": "production" }</pre>${variables.map((v) => i18nHTML`<div class="token-row" data-variable="${esc(v.key)}"><div><strong>${esc(v.key)}</strong> <span class="pill">${v.enabled ? i18nText("已启用") : i18nText("已暂停")}</span><p>${esc(v.environment)} · ${v.secret ? i18nText("密钥 · 日志脱敏") : i18nText("普通变量")} · ${v.protected ? i18nText("仅受保护分支") : i18nText("允许未保护分支")}</p><p class="muted">分支：${v.refs.map(esc).join("、")} · 版本 ${v.revision}</p></div><div class="actionbar">${button(i18nText("编辑"), "edit", v.id)}${button(v.enabled ? i18nText("暂停") : i18nText("启用"), "toggle", v.id)}${button(i18nText("接管"), "own", v.id)}${button(i18nText("删除"), "delete", v.id)}</div></div>`).join("") || i18nText('<p class="empty">暂无变量</p>')}</div><form class="form" id="variable-config"><h3 id="variable-form-title">新建变量</h3><label>变量名<input name="key" required pattern="[A-Z_][A-Z0-9_]{0,79}" placeholder="API_TOKEN"></label><label>环境<input name="environment" required value="*"></label><label>允许的分支<input name="refs" required value="${esc(branch)}"></label><p class="hint">多个分支用逗号分隔，* 表示全部分支。</p><label>值<input name="value" type="password" required maxlength="8192" autocomplete="new-password"></label><p class="hint">密钥至少 8 个字符。编辑时留空保留原值。取消日志脱敏也不会开放值的读取接口。</p><label class="check"><input type="checkbox" name="secret" checked> 密钥（日志脱敏）</label><label class="check"><input type="checkbox" name="protected" checked> 仅允许已启用合并请求保护的分支</label><label class="check"><input type="checkbox" name="enabled" checked> 启用变量</label><p class="hint">修改、暂停、接管、删除或撤销所有者权限会取消已使用该变量的未完成运行。接管后需手动启用。脱敏用于减少意外日志泄露，产物内容由构建代码决定。</p><div class="actionbar"><button class="btn primary" type="submit">保存变量</button>${button(i18nText("清空编辑"), "reset")}</div></form></section>`,
    bind() {
