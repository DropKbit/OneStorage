...
export function issueCard(i, base, esc, select = false, board) {
  return `<article class="issue-card" ${board ? `draggable="${board.writable}" data-issue-id="${i.id}" data-revision="${i.revision}" data-column="${esc(board.column)}"` : ""}>${select ? `<input type="checkbox" aria-label="选择 Issue ${i.id}" data-select-issue="${i.id}" data-revision="${i.revision}">` : ""}<div><a data-link class="subject" href="${base}/issues/${i.id}">${esc(i.title)}</a><p class="muted">#${i.id} · ${esc(i.author)} · ${i.state === "open" ? "开放" : "已关闭"}${i.assignee ? " · 指派给 " + esc(i.assignee) : ""}${i.milestone ? " · " + esc(i.milestone) : ""}</p><div class="issue-labels">${i.labels.map((l) => `<span class="pill" style="border-color:#${/^[a-f0-9]{6}$/i.test(l.color) ? l.color : "64748b"}">${esc(l.name)}</span>`).join("")}</div>${
    board?.writable
      ? `<label class="muted">移动到<select data-move-issue="${i.id}" data-revision="${i.revision}" data-from="${esc(board.column)}"><option value="">选择列</option>${board.columns
...
export async function issuesPage(r, base, ap, h) {
  ...
  const labelChecks = (prefix, selected = []) =>
    planning.labels
      ...
      .join("");
  const filter = `<form class="form panel issue-filters" id="issue-filters">${field("搜索标题和描述", "q", "text", params.get("q") || "")}<div class="inline">${
    !isBoard
      ? `<label>状态<select name="state">${[
          ...
          .join("")}</select></label>`
      : ""
  }${field("作者（用户名或 me）", "author", "text", params.get("author") || "")}${field("负责人（用户名、me 或 none）", "assignee", "text", params.get("assignee") || "")}<label>里程碑<select name="milestone">${option("", "全部", params.get("milestone"))}${option("none", "未分配", params.get("milestone"))}${planning.milestones.map((m) => option(m.id, m.title, params.get("milestone"))).join("")}</select></label><label>排序<select name="sort">${[
    ["newest", "最新创建"],
    ["oldest", "最早创建"],
    ["updated", "最近更新"],
  ]
  ...
  const header = `<div class="titlebar"><h2>Issues ${isBoard ? "看板" : ""}</h2><a data-link class="btn" href="${base}/issues?${query}">${isBoard ? "列表视图" : "看板视图"}</a></div>`;
  const bulk =
    writable(r) && !isBoard
      ? `<details class="panel"><summary class="panelhead">批量更新本页勾选的 Issue（最多 50 个）</summary><form id="issue-bulk" class="form"><label>状态<select name="state">${option("", "保持不变")}${option("open", "重新打开")}${option("closed", "关闭")}</select></label>${field("负责人（留空保持不变，none 清空）", "assignee")}<label>里程碑<select name="milestone_id">${option("", "保持不变")}${option("none", "清空")}${planning.milestones.map((m) => option(m.id, m.title)).join("")}</select></label><details><summary>添加标签</summary>${labelChecks("add")}</details><details><summary>移除标签</summary>${labelChecks("remove")}</details><button class="btn" type="submit">更新所选 Issue</button><p class="muted">如果任一 Issue 已被修改，整批操作会停止，请刷新后重新选择。</p></form></details>`
      : "";
  let body;
  const columns = isBoard
    ...
    : [];
  if (isBoard) {
    body = `<div class="panel form"><label>看板<select id="choose-board">${boards.boards.map((b) => option(b.id, b.name, boardId)).join("")}</select></label>${
      maintain(r)
        ? `<details><summary>${boardId === "default" ? "创建标签看板" : "编辑或创建看板"}</summary><form class="form" id="board-settings">${field("看板名称", "name", "text", boardId === "default" ? "" : data.name)}<div>${labelChecks(
            "column",
            (data.labels || []).map((l) => l.id),
          )}</div><button class="btn" type="submit">${boardId === "default" ? "创建" : "保存此看板"}</button>${boardId !== "default" ? '<button class="btn" type="button" id="new-board">另建看板</button><button class="btn danger" type="button" id="delete-board">删除此看板视图</button>' : ""}<p class="muted">最多 12 个标签列；删除视图保留所有 Issue。关闭卡片会保留标签，移至待处理会移除本看板的标签。</p></form></details>`
        : ""
    }</div><div class="issue-board">${columns.map((c) => `<section class="board-column" data-drop-column="${esc(c.id)}"><h3>${esc(c.name)} <span data-count="${esc(c.id)}"></span></h3><div data-cards="${esc(c.id)}" class="board-cards"><p class="muted">读取中…</p></div><button class="btn small" data-more="${esc(c.id)}" hidden>加载更多</button></section>`).join("")}</div>`;
  } else {
    const next = new URLSearchParams(params);
    if (data.next_cursor) next.set("cursor", data.next_cursor);
    body = `${bulk}<section class="panel"><div class="panelhead"><strong>${data.total} 个匹配 Issue · ${data.open} 个开放</strong>${writable(r) ? '<label><input type="checkbox" id="select-all-issues"> 选择本页</label>' : ""}</div>${data.issues.map((i) => issueCard(i, base, esc, writable(r))).join("") || '<p class="detail-body muted">没有符合筛选的 Issue。</p>'}${data.next_cursor ? `<a data-link class="btn" href="${base}/issues?${next}">下一页 →</a>` : ""}</section>`;
  }
  repoLayout(
    r,
    "issues",
  ...
  async function load(column, cursor) {
    ...
    if (!root.children.length)
      root.innerHTML = '<p class="muted">暂无 Issue</p>';
