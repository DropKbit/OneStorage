...
export async function projectPage(r, base, ap, h, tab, sub) {
  ...
  if (tab === "releases") {
    ...
    repoLayout(
      r,
      "releases",
      `<div class="titlebar"><h2>版本发布</h2></div>${releases.map((x) => `<section class="panel"><div class="panelhead"><strong>${esc(x.title)} · ${esc(x.tag)}</strong><span>${x.prerelease ? "预发布" : "正式版"}</span></div><div class="detail-body"><pre class="prose">${esc(x.body)}</pre><p><code>${x.sha}</code> · ${esc(x.author)}</p>${button("下载源码", "download", x.sha)} ${maintain(r) ? button("删除发布", "delete", x.id) : ""}</div></section>`).join("") || '<div class="empty">从已有 Git 标签发布一个版本。</div>'}${maintain(r) ? `<form class="panel form" id="release">${field("已有标签", "tag")}${field("发布标题", "title")}${textarea("更新说明", "body")}<label><input type="checkbox" name="prerelease"> 预发布版本</label><button class="btn primary" type="submit">发布版本</button></form>` : ""}`,
    );
