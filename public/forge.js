export async function keyPage(h) {
  ...
  const list = (keys, type) =>
    keys
      .map(
        (k) =>
          `<div class="token-row"><div><strong>${esc(k.name)}</strong><p class="muted">${esc(k.algorithm || k.fingerprint)}</p></div><button class="btn small danger" data-key="${esc(type + "/" + k.id)}">撤销</button></div>`,
      )
      .join("");
  layout(
    `<div class="titlebar"><div><h1>密钥与连接</h1><p class="muted">为自动化签发短期权限，连接上游代码仓库。</p></div></div><div class="forge-grid"><section class="panel"><div class="panelhead"><strong>JWT API 公钥</strong></div>${list(apiKeys, "api-keys")}<form id="api-key" class="form"><p class="muted">客户端保留私钥；OneStorage 仅保存公钥。撤销后，现有 JWT 立即失效。</p>${field("名称", "name")}${textarea("SPKI 公钥 PEM", "public_key")}<div class="field"><label>签名算法</label><select name="algorithm"><option>ES256</option><option>ES384</option><option>ES512</option><option>RS256</option></select></div><button type="submit" class="btn primary">注册公钥</button></form></section><section class="panel"><div class="panelhead"><strong>提交签名公钥</strong></div>${list(signing, "signing-keys")}<form id="signing-key" class="form"><p class="muted">用于 verify-sig 引用策略，支持 SSH 与 OpenPGP。</p>${field("名称", "signing-name")}${textarea("SSH / OpenPGP 公钥", "signing-public-key")}<button type="submit" class="btn primary">注册签名公钥</button></form></section></div><section class="panel"><form id="github-app" class="form"><h2>GitHub App</h2><p class="muted">${github.configured ? "已配置 · App " + esc(github.app_id) : "尚未连接"}。安装令牌按仓库申请，私钥和 Webhook 密钥加密存储。</p>${github.webhook_url ? `<code>${esc(github.webhook_url)}</code>` : ""}${field("App ID", "app_id", "text", github.app_id || "")}${field("Installation ID", "installation_id", "text", github.installation_id || "")}${textarea("App 私钥 PEM", "private_key")}${field("Webhook 密钥", "webhook_secret", "password")}<div class="inline"><button type="submit" class="btn primary">保存连接</button>${github.configured ? '<button type="button" class="btn danger" id="disconnect-github">断开连接</button>' : ""}</div></form></section><section class="panel"><div class="form"><h2>Agent 接入</h2><p>MCP 地址：<code>${esc(location.origin + "/mcp")}</code></p><p class="muted">使用 Bearer PAT 或短期 JWT。工具调用沿用仓库权限和引用策略。</p><a href="/llms.txt" target="_blank">机器可读说明 ↗</a></div></section>`,
    "密钥与连接",
    "keys",
  );
...
export async function forgePage(r, base, ap, h) {
  ...
  repoLayout(
    r,
    "forge",
    `<div class="panel"><div class="panelhead"><strong>${ephemeral ? "临时命名空间" : "主命名空间"}</strong><a class="btn small" data-link href="${esc(base + "/forge?ephemeral=" + !ephemeral)}">切换到${ephemeral ? "主" : "临时"}命名空间</a></div><div class="form"><code>${esc(r.clone_url.replace(".git", ephemeral ? "+ephemeral.git" : ".git"))}</code><p class="muted">${ephemeral ? "临时引用与主分支隔离；合并时选择临时来源即可保留成果。" : "Git push、API 编辑和合并使用统一的引用策略。"}</p></div></div><div class="forge-grid"><section class="panel"><div class="panelhead"><strong>分支 · ${branches.branches.length}</strong></div>${branches.branches.map((b) => `<div class="token-row"><div><strong>${esc(b.name)}</strong><p><code>${esc(b.sha.slice(0, 12))}</code></p></div>${write ? `<button class="btn small danger" data-branch="${esc(b.name)}" data-sha="${esc(b.sha)}">删除</button>` : ""}</div>`).join("")}${write ? `<form id="create-branch" class="form">${field("新分支", "target_branch")}${field("起点分支、标签或 SHA", "base_ref", "text", r.default_branch)}${check("base_is_ephemeral", "起点来自临时命名空间", ephemeral)}<button class="btn primary" type="submit">创建分支</button></form>` : ""}</section><section class="panel"><div class="panelhead"><strong>标签 · ${tags.tags.length}</strong></div>${tags.tags.map((t) => `<div class="token-row"><strong>${esc(t.name)}</strong><code>${esc(t.sha.slice(0, 12))}</code>${write ? `<button class="btn small danger" data-tag="${esc(t.name)}">删除</button>` : ""}</div>`).join("")}${write ? `<form id="create-tag" class="form">${field("标签名", "tag-name")}${field("起点", "tag-ref", "text", r.default_branch)}<button class="btn primary" type="submit">创建标签</button></form>` : ""}</section></div><section class="panel"><form id="merge-preview" class="form"><h2>合并预览</h2><div class="forge-grid">${field("来源分支 / SHA", "source_ref")}${field("目标分支", "target_branch", "text", r.default_branch)}</div>${check("source_is_ephemeral", "来源为临时分支", ephemeral)}${check("target_is_ephemeral", "目标为临时分支")}<div class="field"><label>策略</label><select name="strategy"><option value="ff_prefer">优先快进，否则三方合并</option><option value="ff_only">仅快进</option><option value="merge">创建合并提交</option></select></div>${check("squash", "压缩为单个提交")}<button class="btn" type="submit">预览变化</button><div id="merge-result"></div></form></section><div class="forge-grid"><section class="panel"><form class="form" id="note"><h2>Git Notes</h2>${field("提交 SHA", "sha")}${field("Notes 引用", "notes_ref", "text", "refs/notes/commits")}${textarea("附注", "note")}<div class="field"><label>操作</label><select name="operation"><option value="read">读取</option>${write ? '<option value="create">创建</option><option value="append">追加</option><option value="delete">删除</option>' : ""}</select></div><button class="btn" type="submit">执行</button><pre id="note-result" class="forge-output"></pre></form></section><section class="panel"><form id="download" class="form"><h2>文件与归档</h2>${field("版本", "ref", "text", r.default_branch)}<div class="field"><label>文件路径（留空下载归档）</label><input name="path"></div><button class="btn" type="submit">下载</button></form><form id="blame" class="form"><h2>逐行历史</h2>${field("文件路径", "path")}${field("版本", "ref", "text", r.default_branch)}<div class="field"><label>行范围（可选，例如 10,+5）</label><input name="range"></div><button class="btn" type="submit">查看 Blame</button><pre class="forge-output" id="blame-result"></pre></form></section></div>`,
  );
  ...
  if (write) {
    ...
    for (const b of document.querySelectorAll("[data-branch]"))
      b.onclick = async () => {
        try {
          if (!confirm("删除分支 " + b.dataset.branch + "？")) return;
  ...
  bindForm("#merge-preview", async (d) => {
    ...
    document.querySelector("#merge-result").innerHTML =
      `<div class="token-display"><strong>${preview.status === "clean" ? "可以合并" : "存在冲突"} · ${esc(preview.result)}</strong><p>目标：<code>${esc(preview.target_tip_sha)}</code></p>${preview.conflict_paths.length ? `<pre>${esc(preview.conflict_paths.join("\n"))}</pre>` : ""}${write && preview.status === "clean" ? '<button class="btn primary" type="button" id="apply-merge">合并此版本</button>' : ""}</div>`;
    document
...
export async function upstreamPage(r, base, ap, h) {
  ...
  repoLayout(
    r,
    "upstream",
    `<div class="forge-grid"><section class="panel"><form id="upstream-config" class="form"><h2>连接上游</h2><p class="muted">双向同步时，普通分支写入先由上游确认。公开仓库导入需手动拉取，临时分支留在 OneStorage。</p><div class="field"><label>提供方</label><select name="provider">${["github", "gitlab", "bitbucket", "gitea", "forgejo", "codeberg", "sourcehut"].map((p) => `<option ${upstream.provider === p ? "selected" : ""}>${p}</option>`).join("")}</select></div>${field("上游所有者 / 组", "owner", "text", upstream.owner || "")}${field("上游仓库", "name", "text", upstream.name || "")}<div class="field"><label>自托管域名（可选，须已获管理员允许）</label><input name="upstream_host" value="${esc(upstream.upstream_host || "")}"></div><div class="field"><label>GitHub 模式</label><select name="mode"><option value="public" ${upstream.mode === "public" ? "selected" : ""}>公开仓库导入</option><option value="app" ${upstream.mode === "app" ? "selected" : ""}>GitHub App 双向同步</option></select></div><button type="submit" class="btn primary">保存上游</button></form></section><section class="panel"><div class="form"><h2>同步状态</h2><span class="pill">${esc(status.status)}</span><p>${esc(status.error || "")}</p><p class="muted">最近成功：${esc(status.synced_at || "尚未同步")}</p><button class="btn primary" id="pull-upstream" ${upstream.provider ? "" : "disabled"}>拉取上游</button><button class="btn" id="refresh-sync">刷新</button></div>${status.jobs.map((j) => `<div class="token-row"><code>${esc(j.id.slice(0, 8))}</code><span>${esc(j.status)}</span><span class="muted">${j.attempts} 次尝试</span></div>`).join("")}<form id="git-credential" class="form"><h3>HTTPS 上游凭据</h3><p class="muted">用于 GitLab、Gitea 等。GitHub App 在「密钥与连接」中配置。</p>${field("用户名", "username", "text", "oauth2")}${field("密码 / Token", "password", "password")}<button class="btn" type="submit">保存凭据</button></form></section></div><section class="panel"><form class="form" id="fork"><h2>创建独立 Fork</h2>${field("新仓库名称", "name", "text", r.name + "-fork")}${field("起点分支 / SHA", "ref", "text", r.default_branch)}<button class="btn" type="submit">创建副本</button></form><form class="form" id="delete-repo"><h2>删除仓库</h2><p class="muted">引用、Git 数据、LFS 和协作记录将被清理。输入项目名称确认。</p>${field("仓库名称", "confirmation")}<button class="btn danger" type="submit">删除仓库</button></form></section>`,
  );
  bindForm("#upstream-config", async (d) => {
    if (!d.upstream_host) delete d.upstream_host;
    await api(ap + "/upstream", { method: "PUT", body: d });
    notice("上游已配置，点击拉取开始同步");
    render();
  });
  ...
  bindForm("#delete-repo", async (d) => {
    if (d.confirmation !== r.name) throw Error("仓库名称不匹配");
