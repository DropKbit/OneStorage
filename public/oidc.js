export async function completeLogin(h) {
  ...
  if (!h.current()) return;
  layout(
    `<section class="panel"><form class="form" id="oidc-complete"><h1>${pending.stage === "mfa" ? "验证你的身份" : "完成账户创建"}</h1><p>已通过 ${esc(pending.provider)} 验证。</p>${pending.stage === "mfa" ? '<label>验证码或恢复码<input name="otp" autocomplete="one-time-code" maxlength="64" required></label>' : '<label>OneStorage 用户名<input name="username" pattern="[a-z0-9][a-z0-9_-]{0,47}" maxlength="48" required></label><p class="hint">新账户使用统一登录，稍后可在账户安全中设置本地密码。已有账户请先用原方式登录，再关联身份。</p>'}<button type="submit" class="btn primary">继续</button><a data-link href="/login">重新登录</a></form></section>`,
    "统一登录",
    "",
  );
...
export function identityPanel(h, data) {
  ...
  return `<section class="panel"><div class="form"><h2>统一登录</h2>${data.identities.map((i) => `<div class="token-row"><span>${esc(i.name)} · ${i.enabled ? "可用" : "已停用"}</span><div class="actionbar">${i.enabled ? `<button class="btn" type="button" data-oidc-reauth="${i.provider_id}">重新验证</button>` : ""}<button class="btn danger" type="button" data-oidc-unlink="${i.id}">解除关联</button></div></div>`).join("") || '<p class="muted">尚未关联身份。</p>'}${proof}${otp}<div class="actionbar">${data.providers
    .filter((p) => !data.identities.some((i) => i.provider_id === p.id))
    .map(
      (p) =>
        `<button class="btn" type="button" data-oidc-link="${p.id}">关联 ${esc(p.name)}</button>`,
    )
    .join(
      "",
    )}</div><p class="hint">关联会验证当前账户和外部身份，不会按邮箱自动合并。解除关联会撤销相应会话和访问令牌；必须保留一种可用登录方式。</p></div></section><section class="panel"><form class="form" id="oidc-password"><h2>${data.has_password ? "修改本地密码" : "设置本地密码"}</h2>${proof}${otp}<label>新密码<input name="new_password" type="password" autocomplete="new-password" minlength="12" maxlength="128" required></label><button class="btn" type="submit">保存并退出全部会话</button></form></section>`;
}
