const scopeNames = {
  read_repository: [
    "读取代码",
    "通过 HTTPS clone/fetch 和下载 LFS；不能推送。",
  ],
  read_package_registry: ["读取软件包", "查看和下载此范围的软件包。"],
  write_package_registry: ["发布软件包", "上传软件包和修改分发标签。"],
  delete_package_registry: [
    "撤回软件包",
    "删除版本；npm unpublish 还需读取软件包权限。",
  ],
};
export async function deployTokensPage(h, { r, base, ap, workspace }) {
  ...
  const when = (n) => (n ? new Date(n).toLocaleString() : "尚未使用");
  const status = (t) =>
    t.revoked_at !== null
      ? "已撤销"
      : t.expires_at <= Date.now()
        ? "已到期"
        : "有效";
  const content = `<section class="panel" aria-label="部署令牌"><div class="panelhead"><strong>${workspace ? esc(space.name) + " · 空间" : "项目"}部署令牌</strong><span>最多 ${data.limit} 个有效令牌</span></div><div class="detail-body"><p>给部署和构建工具授予指定范围的权限。令牌属于${workspace ? "当前空间，覆盖空间中的所有项目和今后创建的项目" : "当前项目"}，不会因创建者退出团队而自动失效。空间所有者或项目维护者可管理；请在交接时检查并撤销不再需要的令牌。</p><p class="muted">代码只读。发布与撤回权限独立选择。归档项目可读取、不可发布；项目跨空间转移会撤销其项目令牌，原空间令牌也不再覆盖该项目。令牌不能访问账户、成员、Issues、CI 配置或管理员 API。</p><div id="deploy-secret" role="status"></div><div id="deploy-token-rows"></div><div class="inline">${offset ? `<a data-link class="btn" href="${pagePath}?offset=${Math.max(0, offset - 50)}">上一页</a>` : ""}${data.next_offset !== null ? `<a data-link class="btn" href="${pagePath}?offset=${data.next_offset}">下一页</a>` : ""}</div></div></section><section class="panel"><form class="form" id="deploy-token-create"><h2>创建部署令牌</h2><label>名称<input name="name" required maxlength="80" placeholder="production-build" /></label><label>认证用户名（可选）<input name="username" maxlength="80" placeholder="自动生成" pattern="[a-zA-Z0-9][a-zA-Z0-9._+-]*" /></label><label>有效天数<input name="days" type="number" value="90" min="1" max="365" required /></label><fieldset><legend>权限范围</legend>${Object.entries(
    scopeNames,
  )
    .map(
      ([key, [name, hint]]) =>
  ...
  const showSecret = (result) => {
    ...
    document.querySelector("#deploy-secret-show").onclick = (e) => {
      ...
      e.currentTarget.textContent = showing ? "隐藏" : "显示";
    };
  ...
  const redraw = () => {
    ...
    for (const form of document.querySelectorAll("[data-deploy-rotate]"))
      form.addEventListener("submit", async (e) => {
        ...
        if (!confirm("轮换后旧令牌立即失效，继续轮换？")) return;
    ...
    for (const button of document.querySelectorAll("[data-deploy-revoke]"))
      button.addEventListener("click", async () => {
        ...
        if (!confirm(`撤销部署令牌 ${t.name}？使用它的自动化将无法继续认证。`))
          return;
        button.disabled = true;
        try {
  ...
  document
    ...
    .addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (!scopes.length) throw Error("至少选择一项权限");
