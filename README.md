...
**自己的代码，自己的空间。** 基于 Cloudflare Workers 的开源 Git 服务，部署于 **[git.1s.hk](https://git.1s.hk)**。AGPL-3.0-only，**v0.10.0 alpha**。
...
| 入站/出站 pack           | 16 MiB；入站最多 2,000 对象                      |
| 请求对象缓存 / 解压预算  | 各 32 MiB，另有临时缓冲区                        |
| 对象图遍历 / 引用 / 深度 | 5,000 / 256 / 64                                 |
| NDJSON 提交              | 48 MiB 传输、32 MiB 解码、每块 4 MiB、每行 6 MiB |
| Diff / 搜索输出          | 4 MiB；搜索另有计算预算                          |
| 活跃仓库不可达对象       | 保留；删除整个仓库才异步清理                     |

输出 pack 未做 delta 压缩，遍历逐对象读取 R2，平台 CPU/内存/子请求限制可能先触发。没有 TB 级测试、总存储配额、跨服务一致备份或生产 SLA。
