...
v0.14 增加随提交固定的仓库 CI 配置、依赖工作流、并行任务、跨任务产物与父工作流合并/发布门禁，见 [使用说明与验收记录](docs/CI-WORKFLOWS-v14.md)。完整 GitLab/Gogs 目标和剩余工作见 [开发路线](docs/ROADMAP.md)。
...
**自己的代码，自己的空间。** 基于 Cloudflare Workers 的开源 Git 服务，部署于 **[git.1s.hk](https://git.1s.hk)**。AGPL-3.0-only，**v0.14.0 alpha**。
